...
struct searchable : container
{
    ...
    auto contains(typename container_type::key_type const & k) const { return this->find(k) != this->end(); } // note : C++20 feature!
    template <typename container_type = container, typename enable_if<has_type_key_type<container_type>::value, int>::type = 0>
    auto get(typename container_type::key_type & k) -> typename container_type::value_type & { return *(this->find(k)); }

    template <typename container_type = container, typename enable_if<!has_type_key_type<container_type>::value, int>::type = 0>
    auto contains(typename container_type::value_type const & v) const { return find(begin(*this),end(*this),v) != end(*this); }
    template <typename container_type = container, typename enable_if<!has_type_key_type<container_type>::value, int>::type = 0>
    auto get(typename container_type::value_type const & v) -> typename container_type::value_type & { return *find(begin(*this),end(*this),v); }
};
