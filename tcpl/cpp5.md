...
Round Style
-----------
round_{toward_zero,to_nearest,_toward_infinity,_toward_neg_infinity};
...
cd;cp;mv;
---------
bitset();bitset(unsigned long long);bitset(string const &);bitset(char const *);
bitset(string,index,num);

----------
Bit Access
----------
bool test(index);bool operator[];

int main()
{
    bitset<4> b("001");
    cout << b << endl;																	// 0001
    for (auto i = 0; i < b.size(); i++) cout << b.test(i); cout << endl;		// 1000
}

---------------
set;reset;flip;
---------------
set();set(i);reset();reset(i);flip();flip(i);
...
------------------------------------------------
Left Shift and Right Shift (logical right shift)
------------------------------------------------
<< <<= >> >>=
...
template <size_t N> unsigned int count(bitset<N> b)
{
	unsigned int n = 0;
	while (b.any()) {
		while (!b.test(0)) { 	// test(0): test if the right most bit is set or not
			b >>= 1;
		}
		n++;
		b >>= 1;
...
========================
int {iscntrl,isprint,isspace,isblank,ispunct,isdigit,isalpha,isalnum,islower,isupper,}(int);
...
----------------------------------
string substr(index,size); operator+();

Exception: throw out_of_range if index > size()
...
-----------------------
c_str();data();copy(char * buffer, size_type size, size_type index);

-------------------
Numeric Conversions
-------------------
int sto{i,l,ll,ul,ull,f,d,ld}(string const &, * index_return, base);
...
{key,value}_compare; hasher; key_equal;
{const}{pointer,reference,{reverse_,local_}iterator}
...
C(C&&);C& operator=(C&&);
C(initializer_list);C& operator=(initializer_list);
C(n,&);C(begin,end);
void C::swap(C&);void swap(C&,C&);
...
Observers
---------
key_comp;value_comp;key_eq;hash_function;

--------
Capacity
--------
empty;size;capacity;max_size;resize;reserve;shrink_to_fit;
...
--------------------
insert;erase;clear;{push,pop}{front,back};emplace_hint;
try_emplace;emplace;emplace_hint;
insert(node);node extract(position);node extrace(value);[since C++17]
...
if (auto [i,r] = m.insert({"A",".."}); !r) { std::cout << "Exist: " << get<1>(i) << std::endl; }
...
rm;cp;mv;tr;sw
--------------
f remove(f,f,&);o copy(i,i,o);o move(i,i,o);o transform(i,i,o,ufunc);
...
Predicates
----------
bool none_of(i,i,upred); bool any_of(i,i,upred); bool all_of(i,i,upred);
...
-------------------------
bool is_permutation(f,f,f,f,bpred); bool next_permutation(b,b,bpred); bool prev_permutation(b,b,bpred);

------------------------------------
Mutation: Rotate & Reverse & Shuffle
------------------------------------
rotate(f,f,f);reverse(b,b);shuffle(r,r);

----
Heap
----
push_heap(r,r,bpred);pop_heap(r,r,bpred);make_heap(r,r,bpred);sort_heap(r,r,bpred);is_heap();is_heap_until();

-----
Merge
-----
o merge(i,i,i,i,o,bpred);inplace_merge(b,b,b,bpred);

---------
Partition
---------
b partition(b,b,upred);bool is_partitioned();partition_point();b stable_partition(b,b,upred);partition_copy();

----------------
Order Statistics
----------------
f min_element(f,f,bpred);f max_element(f,f,bpred);(f,f) minmax_element(f,f,bpred);
& min(&,&,bpred);& max(&,&,bpred);(&,&) minmax(&,&,bpred);
& min(initializer_list,bpred);& max(initializer_list,bpred);(&,&) minmax(initializer_list,bpred);
nth_element(r,r,r,bpred);

----
Sort
----
sort(r,r,bpred);stable_sort(r,r,bpred);partial_sort(r,r,r,bpred);is_sorted(f,f,bpred);
f is_sorted_until(f,f,bpred);

-----------------------------
Bounds & Equal Range & Unique
-----------------------------
f lower_bound(f,f,&,bpred);f upper_bound(f,f,&,bpred);(f,f) equal_range(f,f,&,bpred);
f unique(f,f,bpred);

-----------------------------------------------------------------------------------
Set Operations: Inclusion & Intersection & Union & Difference & SymmetricDifference
-----------------------------------------------------------------------------------
